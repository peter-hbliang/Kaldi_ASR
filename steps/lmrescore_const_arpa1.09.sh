...
oldlang=`dirname $oldlang`
newlang=`dirname $newlang`
...
if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $outdir/log/rescorelm.JOB.log \
    ...
    lattice-lmrescore-const-arpa --lm-scale=1.09 \
...
if ! $skip_scoring && [ $stage -le 2 ]; then
  ...
  local/score_noweight.sh --cmd "$cmd" $scoring_opts $data $newlang $outdir
else
