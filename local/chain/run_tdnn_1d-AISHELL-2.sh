...
train_stage=-10
get_egs_stage=-10
dir=exp/chain_tdnn_AISHELL-2  # Note: _sp will get added to this
decode_iter=
...
gmm=mfcc_tccainer_sp/tri4a

#train_set=tcc300_aishell_ner_sp/train
train_set=AISHELL-2
test_set=tcc300_fbank/test
ali_dir=exp/${gmm}-ali
lat_dir=exp/${gmm}-lat-AISHELL-2
dir=exp/chain-tdnn-AISHELL-2
train_data_dir=data/AISHELL-2
#lores_train_data_dir=data/tcc300_mfcc_syl/train
lores_train_data_dir=data/tcc300_aishell_ner_sp/train_mfcc
treedir=data/tree-chain-tccainer
...
if [ $stage -le 7 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "run.pl" data/$train_set \
...
if [ $stage -le 9 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd run.pl 3000 $lores_train_data_dir $lang $ali_dir $treedir
fi
...
if [ $stage -le 10 ]; then
  ...
  #affine_opts="l2-regularize=0.01 dropout-proportion=0.0 "
...
if [ $stage -le 12 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 $lang $treedir $dir/graph
fi
...
if [ $stage -le 13 ]; then
      ...
      time steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 10  \
      $graph_dir data/${test_set} $dir/decode_tcc300_syl || exit 1;
