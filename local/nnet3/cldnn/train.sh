...
if [ $stage -le 12 ]; then
  ...
  fixed-affine-layer name=lda input=Append(-4,-3,-2,-1,0,1,2,3,4) delay=5 affine-transform-file=$dir/configs/lda.mat
  relu-renorm-layer name=affine dim=120

  conv-relu-batchnorm-layer name=cnn1 input=affine height-in=40 height-out=20 height-subsample-out=2 time-offsets=-1,0,1 height-offsets=-1,0,1 num-filters-out=256 max-change=0.25
  ...
  relu-renorm-layer name=rm_dim dim=256
  ...
  output-layer name=output input=dnn2 output-delay=5 dim=1569 max-change=1.5
